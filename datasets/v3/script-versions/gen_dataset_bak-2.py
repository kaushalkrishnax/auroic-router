...
CONVERSATION_MEMORY_COUNT = 250   # ~250 coherent conversation flow samples
CPA_PERMUTATIONS = 3              # permutations per sample for Context Permutation Augmentation
...
CONVERSATION_MEMORY_TOPICS = [
    ...
    (["yaar bore ho raha hun","game khel","nahi kuch aur chahiye","movie dekh","bhai ek cute dog gif bhej de please"], "media", 5, "medium", "media:dog_gif"),
    (["bhai feeling low hai aaj","kya hua","kuch nahi bus mood nahi hai","ice cream kha","yaar ek sad sticker bhej de bhai"], "media", 5, "low", "media:sad_sticker"),
    # -- React conversations
    (["bhai ek baat batau","haan bata","bhai selection ho gayi meri","konsi cheez mein","UPSC bhai UPSC clear kar liya"], "react", 5, "null", "🥳"),
    (["yaar sun na kuch hua","kya hua bata","bhai accident ho gaya","kya sach mein","haan bhai hospital mein hun"], "react", 3, "null", "😱"),
    (["bhai gaming tournament ki news","kya hua","bhai hum finals mein aa gaye","serious","haan bhai first time"], "react", 3, "null", "🔥"),
    # -- Acknowledge conversations  
    (["bhai kaam ho gaya","konsa kaam","assignment submit kar di","accha","haan bhai deadline se pehle"], "acknowledge", 1, "null", "noted done"),
    (["yaar ghar pahunch gaya","safe","haan bhai safe hun","accha","trip mast rahi"], "acknowledge", 1, "null", "glad youre safe"),
    (["bhai payment kar di","kitni","poori amount","upi se","haan bhai confirm kar le"], "acknowledge", 1, "null", "payment received"),
    # -- Translate conversations
    (["bhai ek message aaya foreign mein","kaunsi language","pata nahi","bhej dikhao","감사합니다 정말 도움이 많이 됐어요"], "translate", 5, "null", "translate:korean_text"),
    (["yaar caption mein kuch likha tha","kya","samajh nahi aaya","screenshot bhej","ありがとうございました本当に助かりました"], "translate", 5, "null", "translate:japanese_text"),
    (["bhai email aaya kisi ka","konsa email","foreign language mein hai","forward kar","Muchas gracias por tu ayuda amigo de verdad"], "translate", 5, "null", "translate:spanish_text"),
...
def generate_reasoning(typ, target, msgs, title=""):
    """Generate a structured <think> block for decision boundary anchoring.
    Keeps reasoning short (10-25 tokens), never leaks the answer directly."""
    t = f"M{target}" if target else "null"

    if typ == "ignore":
        templates = [
            "messages contain only spam noise and filler no actionable request",
            "all messages are emoji reactions or gibberish nothing to route",
            "conversation is just laughter and filler ignore",
            "no real request present only noise and spam messages",
            "chain of spam or joke messages no actionable signal found",
        ]
        return random.choice(templates)

    if typ == "translate":
        templates = [
            f"foreign language text detected in {t} translation needed",
            f"non-english non-hindi script appears in {t} requires translation",
            f"message {t} contains foreign text that needs to be translated",
            f"unrecognized language script found in {t} translate request",
        ]
        return random.choice(templates)

    if typ == "media":
        templates = [
            f"user explicitly asked for gif or media content in {t}",
            f"media request detected in {t} user wants visual content sent",
            f"message {t} is asking for sticker or gif to be sent",
            f"clear media send request found in {t} other messages are filler",
        ]
        return random.choice(templates)

    if typ == "react":
        templates = [
            f"message {t} shares news or event that deserves emotional reaction",
            f"user sharing something exciting or noteworthy in {t} react appropriate",
            f"emotional or celebratory content in {t} warrants reaction emoji",
            f"significant personal event shared in {t} reaction is appropriate",
        ]
        return random.choice(templates)

    if typ == "acknowledge":
        templates = [
            f"user sharing status update or completion info in {t} acknowledge",
            f"informational message in {t} no action needed just acknowledgment",
            f"message {t} is a status update that requires simple acknowledgment",
            f"user providing update or greeting in {t} respond with acknowledgment",
        ]
        return random.choice(templates)

    # text type — most common, needs varied reasoning
    if target:
        # Check if there are competing signals
        has_competing = False
        for i, m in enumerate(msgs):
            if i != target - 1 and len(m.split()) > 4:
                has_competing = True
                break

        if has_competing:
            templates = [
                f"multiple messages look actionable but {t} has the strongest help request",
                f"competing signals present but primary request is in {t}",
                f"several messages have content but {t} contains the actual request",
                f"filler and noise around {t} but the real help request is there",
            ]
        else:
            templates = [
                f"help request detected in {t} other messages are casual filler",
                f"actionable request found in {t} surrounding messages are noise",
                f"user asking for assistance in {t} rest is conversation filler",
                f"technical or personal help request in {t} route there",
            ]
        return random.choice(templates)

    return "user asking for help route to target"
...
def generate_sample(type_hint=None, lang_hint=None, force_adversarial=False, force_multi_signal=False):
    ...
    # Generate thinking block for decision boundary anchoring
    reasoning = generate_reasoning(type_hint, target_int, msgs, parts.get("TITLE",""))
...
def main():
    ...
    for s in samples:
        ...
        for _ in range(CPA_PERMUTATIONS):
            ...
            # Regenerate reasoning for the permuted context
            target_int = (target_idx + 1) if target_idx is not None else None
            reasoning = generate_reasoning(typ, target_int, perm_msgs)

            # Extract original R: decision line
            orig_content = s["messages"][2]["content"]
            r_lines = [l for l in orig_content.split("\n") if l.startswith("R: ")]
            dec_line = r_lines[0] if r_lines else ""
    ...
    aug_out = "dataset_v3_augmented.jsonl"
    ...
    for i, s in enumerate(samples):
        ...
        # Extract the R: decision line (skip <think> block)
